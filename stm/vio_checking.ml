...
let check_vio_depends (ts,f) fmt delim =
  ...
  List.fold_left (fun acc ids -> Stm.check_task_depends f disch tasks fmt delim ids && acc) true ts
...
let schedule_vio_depends_task_checking j tsfs =
  ...
  List.iter (fun (ts, f) ->
    ...
    let infos = List.filter (fun (_,_,i) -> List.mem i ts) infos in
...
let schedule_vio_task_checking j tsfs =
  if j < 1 then Errors.error "The number of workers must be bigger than 0";
  let jobs = ref [] in
  List.iter (fun (ts, f) ->
    let f =
  ...
  let rec filter_argv b = function
    | [] -> []
    | "-schedule-vio-task-checking" :: rest -> filter_argv true rest
    | s :: rest when String.length s > 0 && s.[0] = '-' && b -> filter_argv false (s :: rest)
    | _ :: rest when b -> filter_argv b rest
    | s :: rest -> s :: filter_argv b rest in
  let pack = function
